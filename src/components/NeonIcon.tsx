...
type IconName =
  | 'layers' | 'bolt' | 'shield' | 'support'
  | 'design' | 'code' | 'responsive' | 'store' | 'seo' | 'hosting'
  | 'chat' | 'rocket' | 'chart' | 'clock' | 'star' | 'lock' | 'sparkles'

const iconPaths: Record<IconName, ReactNode> = {
  layers: <path d="m12 3 8 4-8 4-8-4 8-4Z M4 12l8 4 8-4M4 17l8 4 8-4" />,
  bolt: <path d="m13 2-9 12h7l-1 8 9-12h-7l1-8Z" />,
  shield: <path d="M12 3 20 6v5c0 5-3.5 8-8 10-4.5-2-8-5-8-10V6l8-3Z m8 12 3 3 3-3" />,
  support: <path d="M4 13v-2a8 8 0 0 1 16 0v2 M4 13H3a2 2 0 0 0 2 2h1v-5H4M20 13h1a2 2 0 0 1-2 2h-1v-5h2M12 19h4" />,
...
export default function NeonIcon({ name, size = 'sm' }: { name: IconName; size?: 'sm' | 'md' }) {
  const dims = size === 'sm' ? 'w-5 h-5' : 'w-7 h-7'
  const boxSize = size === 'sm' ? 'icon-box-sm' : 'icon-box-md'
  return (
    <div className={`${boxSize} icon-box`}>
      <svg
        viewBox="0 0 24 24"
        fill="none"
        stroke="currentColor"
